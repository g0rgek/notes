...
- Процесс - приложение, загруженное в оперативку, код внутри взят на исполнение. У каждого процесса своё адресное пространство [с секциями](<Стек горутины.md#Общее объяснение>). Каждый процесс представлен в виде TASK в планировщике для переключения между ними.
...
Плюсы - горутина снимается с исполнения только в безопасные моменты для  [[GC]]. Но если горутина будет крутиться в бесконечном цикле, выполняя cpu-intensive задачи без блокировок - то передачи управления не будет.
...
1. Не давать разработчику потоки, а только горутины. Они [легковесные](<goroutine.md#Что такое горутина ?#Легковесный поток>).
...
- В итоге, одна из горутин будет вытеснена в [[#GRQ]]
# Work stealing
scheduler_tick - счетчик, увеличивается на 1 когда планировшик ставит новую горутину на исполнение.
## Алгоритм
Если LRQ у P опустела, он: 
0. Если это 1/61 scheduler_tick, то P заберет половину из [[#GRQ]]. Если нет - к шагу 1.
1. рандомно P, чтобы забрать **с конца** его [[#LRQ]] **половину** горутин. Это позволяет минимизировать contention rate и забрать достаточное количество работы себе и оставить изначальному P. LRQ представлены в виде Lock-free очереди, без [[mutex]].
2. Если не получилось украсть - он попробует украсть еще 4 раза.
3. Если опять не получилось - идет в GRQ
4. Проверит [[#Netpoller]] и заберет готовые горутины
## Запуск алгоритма
...
Так как планировщик реализует [[#Кооперативная многозадачность]], когда горутина переходит в статус waiting, у нее выставляется определенный
```go
type waitReason uint8
```
# GRQ
- Если горутина не смогла вернуться в [[#LRQ (local runqueue)]] своего/рандомного P - она попадает в Global Run Queue
- Если горутина исполняется дольше 10ms - [[#Sysmon]] вытесняет её в конец GRQ
- Доступ к GRQ синхронизируется через [[mutex]]
- P ходят в GRQ каждый 1/61 тик планирования [[#Work stealing]]
# Sysmon
Принимает решения в планировщике:
- Следит за временем выполнения горутин, отправляет SIGURG для вытеснения, помещает в [[#GRQ]]
- Производит handoff при [[#Synchronous syscall]] 
- Собирает метрики
- Еще что-нибудь...
# Synchronous syscall
...
2. [[#Sysmon]] понимает, что горутина блокируется на сисколе, снимает (OS thread-M) с логического процессора P. 
...
- Горутины снимаются с выполнения, передаются [[#Netpoller]]
# Netpoller
## Visual
```
                      sock│                               
