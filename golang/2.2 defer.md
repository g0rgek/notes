...
defer по своей природе является такой же структурой (как [slice](slice.md), [interface](interface.md), [chan](chan.md) и т.д.). В случае defer в цикле происходит [escaping](Escape%20analysis.md) структур в кучу:
```go
package main
...
- **Не** выполняет `defer` в какой бы то ни было горутине. Также [GC#Финализаторы](5.%20GC.md#Финализаторы.md) не гарантируются к выполнению, сборщик мусора не будет принудительно запускаться, буферы стандартного вывода/записи могут остаться неслитыми.
