...
```
Эти функции буду выполнены в обратном порядке. Т.е. мы кладем вызовы на стек и выполнение начинается с последнего вызванного `defer`.
# Аргументы defer
...
```
### Пример 2
Если изменить [получателя](./types/struct.md#Методы%20структуры) на указатель, то напечатает "bar" (иначе "foo"):
```go
package main
...
[zero value](1.3%20zero%20value.md) для функций является [nil](./types/nil.md). Очевидно, если передать nil в `runtime.deferproc()`, то при вызове получим `nil pointer dereference`:
```go
package main
...
## Именованные
У функций могут быть [именованные возвращаемые результаты](./types/function.md#Именованные%20возвращаемые%20результаты). Defer умеет модифицировать такие аргументы.
По сути, это [замыкание захватывает по указателю](./memory%20model/Замыкание%20захватывает%20по%20указателю.md) и изменяет значение.
```go
package main
...
defer по своей природе является такой же структурой (как [slice](./types/slice.md), [./types/interface](interface.md), [chan](./concurrency/chan.md) и т.д.). В случае defer в цикле происходит [escaping](./memory%20model/Escape%20analysis.md) структур в кучу:
```go
package main
...
```
# Defer inlining
С версии `go 1.14` появилось [встраивание](./types/function.md#Встраивание функций) defer в простых случаях.
...
| [panic и recover](2.1%20panic%20и%20recover.md) | Да (defer выполняются при распутывании стека) | Если не перехвачен — завершает горутину и, при достижении вершины, печатает стек и завершает процесс | `recover` в `defer` может перехватить |
...
- **Не** выполняет `defer` в какой бы то ни было горутине. Также [финализаторы](5.%20GC.md#Финализаторы) не гарантируются к выполнению, сборщик мусора не будет принудительно запускаться.
