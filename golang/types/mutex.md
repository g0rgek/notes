...
- Если [[goroutine]] попробует захватит **УЖЕ** захваченный mutex, то она поменяет свое состояние с `Running -> Waiting`.
...
# WaitQueue
Внутри mutex используется очередь заблокированных горутин. Она представлена как сбалансированное дерево. Реализована [структурой sudog](<chan.md#sendq & recvq>), которая используется в waitq/recvq у каналов.
```go
