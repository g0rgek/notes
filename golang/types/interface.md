...
Заменяют [Полиморфизм](Полиморфизм.md) из [1.1 OOP](1.1%20OOP.md).
...
```
# Duck typing
В отличие от Rust, в Go нет отдельного `impl` блока, в котором описываются все [методы структуры](struct.md#Методы%20структуры). Если сигнатуры методов класса совпадают с теми, что указаны в интерфейсе - класс начинает удовлетворять этому интерфейсу:
```go
package main
...
```
## Сравнивание
Нужно помнить, что есть [1.2 uncomparable types](1.2%20uncomparable%20types.md) типы, для которых не определен оператор `==`.
Напомним, что [slice несравниваемый тип](slice#Slice%20is%20not%20comparable%20(cant%20use%20as%20key%20in%20map)) и является одним из таких. Но он удовлетворяет типу `any (пустой интерфейс)`. На этапе компиляции мы не узнаем об ошибке. Но в рантайме, при сравнении слайсов, произойдет [паника](2.1%20panic%20и%20recover.md#panic>).
```go
package main
...
Можно использовать синтаксис `val, ok := variable.(Type)` чтобы вытащить из интерфейса его динамический тип или привести один интерфейс к другому. Если статический `Type` указан правильный, то значение динамического **копируется в переменную** и флаг `ok` будет true.  Если нет - [nil](nil.md) и `false`.
Важно понимать, что проверка происходит в рантайме, c использованием [reflection](8.%20Reflection.md). Если не использовать флаг `ok`, то будет паника.
```go
package main
...
`variable.(Type)` проверяет, что `variable != nil`, так как [empty interface](#Empty%20interface%20(any)) может не иметь динамического типа и значения. 
И если:
...
# Type switch
## Определение
Можно использовать [type assertion](#Type%20assertion) вместе со `switch`. Тогда вариантами выступают типы:
```go
package main
...
Можно через `for..range`, можно через [unsafe](6.%20Unsafe%20package.md).  По сути нужно поменять тип `ReadWriter` на `Reader`, и это безопасно.
```go
package main
