...
Если первым полем добавить неименованое поле типа [[#Пустая структура]] (занимает 0 байт), то мы не сможем инциализировать структуру без имен полей.

**ВАЖНО**: в силу [обработки struct{}](<#Пустая структура#0 байт всегда>), если анонимное поле поставить в конец структуры - компилятор добавит 8 байт к размеру (1 байт структуры и 7 для [выравнивания](<#Allignment>)). Поэтому поле нужно ставить **ПЕРВЫМ** по списку. Оно будет иметь такой же адрес, как и ненулевое поле после него.
```go
package main
type Config struct {
...
```
# Область видимости полей ([[Инкапсуляция]])
...
- **иероглиф** - приватное!!!
Замена [[Инкапсуляция]] из [[OOP]].
# Встраивание типов ([[Наследование]])
## Типа наследование
...
Является заменой [[Наследование]] из [[OOP]].
```go
type Event struct {
    ID int
    time.Time
...
Сначала мы определяем для какого типа будет этот метод (значение/указатель), затем идет стандартная сигнатура [[function]]:
...
```
Прочтите чем отличается [[Ресивер по значению и по указателю]].
# Анонимные структуры
...
# Union
## SBO
Предположим, что маленькие массивы мы хотим аллоцировать на [[Стек горутины]], а большие в куче.
...
```
## C-style
Похоже на [[slice#Slice transformation]].
```go
package union
...
```
# Тэги полей
У полей могут быть теки (набор пар ключ-значение). Они опциональны, парсятся в AST и нужны для правильного парсинга в разных форматов в Гошные структуры([[Reflection]]):
...
С помощью [[Unsafe package]] можно кастит type definition к underlying type:
```go
package main
...
В этом примере, самое большое поле структуры - int32, занимает 4 байта. Значит каждое поле будет выровнено до четырёх байт. И порядок размещения полей может влиять на итоговый размер структурыю Узнать размер структуры можно, используя unsafe.[[Sizeof]]:
```go
package main
...
## Приколы со [[Стек горутины]]
В стеке Go может дать разным переменным разные адреса, даже если они zero-sized.
Переменные `a` и `b` лежат на [[Стек горутины]] main, но проверка равности их адресов возвращает false.

А для `s1` и `s2` произошел перенос в кучу за счет Printf -> применилась оптимизация и переменным был присвоен общий фиктивный [[#zerobase - единый адрес на всех]].
```go
package main
...
```
  ...
  Поскольку сравнение [[array]] использует семантику равенства указателей, оно может вернуть значение true, если считает указатели равными, даже если соотношение &a == &b непредсказуемо. Ключевой вывод: **равенство указателей на пустые структуры изначально ненадёжно**.
### arm64
```asm
