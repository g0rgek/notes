...
Важно понимать, что в функции `subscribe()`, при вызове `c.Wait()` [mutex](mutex.md) разблокируется "под капотом". Пока горутина спит, нет смысла в захвате мьютекста. Когда в `publish()` вызовется `Broadcast()` и разбудятся горутины - они снова захватят мьютекс и продолжат выполнение.
```go
package main
...
```
# Semaphore 
Аналогичная логика [chan#Simple semaphore](chan#Simple%20semaphore.md)
