Когда присваивается значение, которое является [[zero value]] для типа - включается оптимизация с использованием `memclr`. Возможно, она использует векторизацию и поэтому оно быстрее.
```go
package main
