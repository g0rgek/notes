...
# Spinlock
Активное ожидание, которое просто так греется процессор ненужным циклом. Если код в критической секции выполняется быстро, то spinlock будет эффективнее чем смена статуса горутины, перевода в [mutex#WaitQueue](mutex.md#WaitQueue.md) и т.д.
