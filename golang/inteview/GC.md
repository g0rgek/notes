...
## Stop The World
[[GMP model#Asynchronous preemptive]] выполнение также играет ключевую роль в сборке мусора:
...
2. **Mark**: 
	...
	5. Начинается конкурентная разметка: 
		...
		1. GC сканирует все [[Стек горутины]] и глобальные переменные на наличие ссылок на объекты в куче. Находит и добавляет в очередь на сканирование, помечает все добавленные объекты серыми.
		2. В цикле:
...
- Нет гарантий работы с [объектами, которые весят 0 байт](<struct.md#Пустая структура>).
```go
...
### Структуры
Если повесить финализатор не на первое поле структуры - будет [паника](<panic и recover.md#panic>):
```go
func main() {
	foo := &Foo{a:1, b: "hello"}
...
- Построена на [[Generics|Generics]], статически проверяет типы параметров функции финализации
