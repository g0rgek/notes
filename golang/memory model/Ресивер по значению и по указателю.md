...
Напомним, что тип [error](../2.%20error.md) - это интерфейс с методом `Error() string`.
# Pointer receiver
Если ресивер является указателем, то [интерфейсу](../types/interface.md) удовлетворяет **ТОЛЬКО** указатель на структуру.
```go
package main
...
Такое работает **ТОЛЬКО** со [СТАТИЧЕСКИМИ](../types/interface.md#Static%20and%20dynamic%20type) типами.
