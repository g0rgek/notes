...
Escape Analysis пытается оставлять всё на [Стек горутины](Стек%20горутины.md) Но в некоторых случаях, переменные убегают в кучу. Строится взвешенный граф, относительно используемых объектов:
- вершины - переменные
...
- Возвращаем из функции структуру с указателем ([slice](slice.md))
- Если одно из полей структуры должно быть в куче- то вся структура улетит в кучу
- Если один из элементов [array](array.md) / [slice](slice.md) улетит в кучу - то все элементы тоже
