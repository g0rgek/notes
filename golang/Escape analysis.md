...
Escape Analysis пытается оставлять всё на [[Стек горутины]] Но в некоторых случаях, переменные убегают в кучу. Строится взвешенный граф, относительно используемых объектов:
- вершины - переменные
...
Метод `Read()` [интерфейса](<interface.md>) `io.Reader` устроен так:
```go
...
Чтобы этого не было, лучше передавать `[]byte` в функцию и записывать в него. Такой подход позволит хранить [[slice]] на стеке.
```go
package main
