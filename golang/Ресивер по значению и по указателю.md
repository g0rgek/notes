...
Напомним, что тип [[error]] - это интерфейс с методом `Error() string`.
# Pointer receiver
...
Такое работает **ТОЛЬКО** со [СТАТИЧЕСКИМИ](<interface.md#Static and dynamic type>) типами [СТРУКТУР](<struct.md>).
...
- ресивер это [[Data types#Примитивные]] (они копируются и так)
