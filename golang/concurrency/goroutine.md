...
## Легковесный поток
  - Динамически расширяемый стек, растет при необходимости
    - [Начальный размер стека](Стек%20горутины.md#Начальный%20размер%20стека) - 2КБ
    - [Максимальный размер стека](Стек%20горутины.md#Максимальный%20размер%20стека) - 265mb / 1gig
    - 1000 горутин - 2MB, 1000 тредов - 8GB
  - Инициализация требует намного меньше инструкций, чем для потоков OS
  - Переключение горутин происходит в [5. GC](5.%20GC.md)-safe моменты. Не нужно сохраняться все состояние процессора (регистры, флаги) для возобновления работы, достаточно ProgrammCounter, StackPointer, InstructionPointer.
...
## Структура языка
 [Сущность языка Go](https://github.com/golang/go/blob/36bca3166e18db52687a4d91ead3f98ffe6d00b8/src/runtime/runtime2.go#L473), представлена [структурой](struct.md). 
  - Управляется [планировщиком Go](GMP%20model.md)
  - Содержит состояние кода, который может исполняться конкуретно
    - Поле для хранения [стека горутины](Стек%20горутины.md)
    - Поле StackGuard - флаг, который выставляется если горутина выполняется дольше 10ms
    - Поле `inMarkAssist` которое показывает помогает ли она раскрашивать кучу [5. GC](5.%20GC.md#Алгоритм).
# Утечки горутин
## 1 variant
Писатель не закрыл [канал](concurrency/chan.md), который [читается](concurrency/chan.md#Чтение) в другой горутине. Поэтому читающая горутина никогда не завершится и будет потреблять ресурсы.
...
Если использовать [небуферезированный канал](concurrency/chan.md#Unbuffered%20chan%20(synchronous)), то одна из горутин успеет записать в канал и функция завершится. Но четыре оставшиеся заблокируются на запись **НАВСЕГДА** 
Чтобы починить:
1. Добавить каналу [буфер](concurrency/chan.md#Buffered%20chan%20(asynchronous)). Это допустимо, так как даже если в канале есть значения, но на него никто не ссылается - [5. GC](5.%20GC.md) его очистит в будущем.
