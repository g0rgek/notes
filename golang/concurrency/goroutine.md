...
Если использовать [небуферезированный канал](chan#Unbuffered%20chan%20(synchronous)), то одна из горутин успеет записать в канал и функция завершится. Но четыре оставшиеся заблокируются на запись **НАВСЕГДА** 
Чтобы починить:
1. Добавить каналу [буфер](chan.md#Buffered%20chan%20(asynchronous)). Это допустимо, так как даже если в канале есть значения, но на него никто не ссылается - [5. GC](5.%20GC.md) его очистит в будущем.
