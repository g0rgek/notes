...
- Процесс - приложение, загруженное в оперативку, код внутри взят на исполнение. У каждого процесса своё адресное пространство [с секциями](Стек%20горутины.md#Общее%20объяснение). Каждый процесс представлен в виде TASK в планировщике для переключения между ними.
...
- В Go - кооперативная многозадачность. По коду расставлены метки, когда горутина может быть безопасная снята с выполнения: 
	...
	- блокировка на [chan](chan.md) или [mutex](mutex.md)
	- time.Sleep
...
Плюсы - горутина снимается с исполнения только в безопасные моменты для  [5. GC](5.%20GC.md). Но если горутина будет крутиться в бесконечном цикле, выполняя cpu-intensive задачи без блокировок - то передачи управления не будет.
...
1. рандомно P, чтобы забрать **с конца** его [LRQ](LRQ) **половину** горутин. Это позволяет минимизировать contention rate и забрать достаточное количество работы себе и оставить изначальному P. LRQ представлены в виде Lock-free очереди, без [mutex](mutex.md).
...
```
# GRQ
- Если горутина не смогла вернуться в [local runqueue](LRQ%20(local runqueue)) своего/рандомного P - она попадает в Global Run Queue
- Если горутина исполняется дольше 10ms - [sysmon](#Sysmon) вытесняет её в конец GRQ
- Доступ к GRQ синхронизируется через [mutex](mutex.md)
