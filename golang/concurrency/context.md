context.Context - это [interface](../types/interface.md).
...
- Вызывайте cancelFunc с [defer](../2.2%20defer.md), чтобы автоматически отменить при выходе из функции
```go
...
Язык позволяет использовать [type definition](../types/struct.md#Псевдонимы%20и%20определения) для нужного типа.
Так как WithValue принимает [empty interface](../types/interface.md#Empty%20interface%20(any)), он сохраняет динамический тип и значение. И перекрытия не будет, так как типы key1 и key2 **РАЗНЫЕ**:
```go
func main() {
